...
fn run_threadpool_coordinator(
    ...
    loop {
        ...
        if cancellation_value {
            ...
            let mut active_tasks_locked = active_tasks.lock()
        ...
        // Check for any finished theads.
        {
            ...
            let threads_until_limit = max_num_threads - active_tasks_locked.len();
...
impl CancellableThreadPool {
    ...
    pub fn join(mut self) -> Result<ThreadPoolStopReason> {
        ...
        coordinator_thread_handle.join()
            .map_err(
                |error|
                    miette!("Threadpool coordinator exited abnormally: {:?}", error)
            )
    }
