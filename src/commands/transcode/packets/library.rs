...
impl<'a> LibraryWorkPacket<'a> {
    pub fn from_library(
        ...
        let (_, artist_directories) = filesystem::list_directory_contents(&library.path)
            .wrap_err_with(|| miette!("Error while listing artists for library: {}!", library.name))?;

        for artist_directory in artist_directories {
            let directory_name = artist_directory.file_name()
                .to_string_lossy()
                .to_string();
            
            // Skip scanning of manually ignored directories in the base folder of the library.
            if let Some(ignored_directories) = &library.ignored_directories_in_base_directory {
                if ignored_directories.contains(&directory_name) {
                    continue;
                }
            }

            let (_, album_directories) = filesystem::list_dir_entry_contents(&artist_directory)
                .wrap_err_with(|| miette!("Error while listing albums for artist: {}!", directory_name))?;
