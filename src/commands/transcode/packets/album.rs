...
impl<'a> AlbumWorkPacket<'a> {
    ...
    pub fn needs_processing(&mut self, config: &Config) -> Result<bool> {
        ...
        if let Some(saved_meta) = saved_meta {
            ...
            let meta_diff = saved_meta
                .diff_with_fresh_or_missing_from_target_dir(
                    &fresh_meta,
                    &self.album_info,
                    config,
                )?;
    ...
    pub fn get_work_packets(
        &mut self,
        config: &Config,
    ) -> Result<Vec<FileWorkPacket>> {
        ...
        if saved_meta.is_some() {
            ...
            for new_file_name in
                diff.files_new.iter().chain(diff.files_untranscoded.iter())
            {
                file_packets.push(FileWorkPacket::new(
        ...
        file_packets.sort_unstable_by(|first, second| {
